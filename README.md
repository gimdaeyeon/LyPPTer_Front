...
### ppt 미리보기 핵심 라이브러리
최초에 Fabric.js를 사용하여 구현하였으나 이후 PptxGenJS 기반 계산식으로 전환
...
**Fabric.js → PptxGenJS 기반 계산식으로 전환한 이유**
- 브라우저 CSS로 렌더링된 미리보기 화면, Fabric.js로 렌더링된 화면, 실제 생성된 PPT 파일의 화면이 서로 일치하지 않는 문제가 발생
- 이 문제를 해결하기 위해 PptxGenJS의 단위 계산식을 기반으로 미리보기 화면을 직접 구성하는 방식으로 변경
- 미리보기와 실제 PPT 파일 간의 레이아웃 일관성 확보
